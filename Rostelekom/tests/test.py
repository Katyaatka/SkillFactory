...
@pytest.mark.parametrize('numbers', [
    ...
    'пустая строка',
    'номер не формата +7ХХХХХХХХХХ',
    'номер не формата +375XXXXXXXXX'
])
@pytest.mark.parametrize('passwords', [
    '', 'Ab1234!', 'abcd12345!', '1ж34567890', 'ABCDEabcde1234567890!'
], ids=[
...
def test_sign_up_with_numbers_negative(browser, numbers, passwords):
    '''Негативный тест. Проверка возникновения ошибок при использовании различных комбинаций номера и пароля.
    Использованы техники тест-дизайна: граничные значения (7 и 21 символы пароля), эквивалентное разбиение (пустые
     строки у номера и у пароля). Также используется параметризация, чтобы протестировать возможные варианты с
     невалидными тестовыми данными'''

    page = SignUp(browser)
    page.sign_up_with_number_and_params(name, last_name, numbers, passwords)
    wait = WebDriverWait(browser, 10)
...
@pytest.mark.parametrize('passwords', [
    '', 'sljgfvkfbvkjsjvnsjvjv484853515vxd'
], ids=[
    'пустая строка',
    'пароль, несоответствующий номеру',
])
def test_login_with_numbers_negative(browser, numbers, passwords):
    '''Негативный тест. Проверка возникновения ошибок при использовании различных комбинаций номера и пароля.
    Используется параметризация, чтобы протестировать возможные варианты с невалидными тестовыми данными'''

    page = LoginPage(browser)
    page.login_with_number_and_params(numbers, passwords)
    elem = browser.find_element(By.XPATH, "//div[contains(@class, 'login-form-container__error--bold')]")
...
def test_login_with_mails_negative(browser, mails, passwords):
    '''Негативный тест. Проверка возникновения ошибок при использовании различных комбинаций почты и пароля.
    Используется параметризация, чтобы протестировать возможные варианты с невалидными тестовыми данными'''

    page = LoginPage(browser)
    page.login_with_mail_and_params(mails, passwords)
    elem = browser.find_element(By.XPATH, "//div[contains(@class, 'login-form-container__error--bold')]")
